...
export default definePluginEntry({
    ...
    async register(api) {
        ...
        savedConfig = config;
        // CRITICAL FIX: Initialize plugin in background using setImmediate (not async/await)
        // This avoids gateway ignoring the plugin registration
        setImmediate(async () => {
            try {
                await ensureInitialized();
                logInfo('Plugin initialized successfully in background');
            }
            catch (error) {
                logError(`Failed to initialize plugin: ${error.message}`);
            }
        });
