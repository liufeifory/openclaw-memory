...
export class DocumentSplitter {
    ...
    groupBySemantics(paragraphs) {
        if (paragraphs.length === 0)
            return [];
        const groups = [[paragraphs[0]]];
        let currentGroupSize = paragraphs[0].length;
        for (let i = 1; i < paragraphs.length; i++) {
            const paragraph = paragraphs[i];
            const prevParagraph = paragraphs[i - 1];
            // Check if we should start a new group
            const shouldStartNewGroup = currentGroupSize + paragraph.length > this.options.maxChunkSize ||
     ...
     */
    splitOversizedChunk(content, source) {
     ...
     */
    mergeSmallChunks(chunks, _source) {
        if (chunks.length <= 1)
            return chunks;
        const merged = [];
    ...
    createChunk(content, index, source, chunkType = 'paragraph') {
        return {
            content,
            index,
...
//# sourceMappingURL=document-splitter.js.map