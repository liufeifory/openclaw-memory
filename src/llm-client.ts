...
export interface LLMOptions {
  ...
  temperature?: number;
  topP?: number;
  model?: string;  // Override default model for specific task
}
...
export class LLMClient {
   ...
   * Build request body
   */
  private buildRequestBody(prompt: string, options: LLMOptions): unknown {
...
export function createLLMClients(config: LLMConfig): {
  localClient: LLMClient;  // Legacy name, now cloud-only
  hybridClient: LLMClient; // Legacy name, now cloud-only
} {
