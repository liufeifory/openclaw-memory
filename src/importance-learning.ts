...
export class ImportanceLearning {
  ...
  private static readonly BASE_WEIGHT = 0.5;
  private static readonly ACCESS_WEIGHT = 0.3;
  ...
  calculate(
    ...
    const importance =
      ImportanceLearning.BASE_WEIGHT * baseImportance +
   ...
   */
  calculateRecencyScore(createdAt: Date, referenceTime: Date = new Date()): number {
    const daysSinceCreation = (referenceTime.getTime() - createdAt.getTime()) / (1000 * 60 * 60 * 24);
   ...
   */
  applyDecay(importance: number, decayFactor: number = ImportanceLearning.DECAY_FACTOR): number {
