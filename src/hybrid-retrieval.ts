...
export class HybridRetriever {
  ...
  mergeResultsWithTopics(
    ...
    const mergedMap: Map<number, MemoryResult> = new Map();
    const memorySources: Map<number, string[]> = new Map();
    ...
    for (const result of allResults) {
      ...
      if (existing) {
        ...
        const existingPrioritized = (existing.score ?? existing.similarity ?? 0) / PATH_PRIORITY[existing.source as keyof typeof PATH_PRIORITY];
      ...
      if (validEntityIds.length > 0 && degree > 1) {
        try {
          ...
          for (const seedMem of seedMemories.slice(0, 3)) {  // Limit to top 3 seeds
            const multiDegreeMemories = await this.entityIndexer.searchByMultiDegree(
              seedMem.id,
              degree,
              0.1,  // minWeight
              INITIAL_K
