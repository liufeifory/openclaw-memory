...
export class SurrealDatabase {
  ...
  private async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        console.error(
          `[SurrealDB] ${operationName} failed (attempt ${attempt}/${this.maxRetries}):`,
          error.message
        );
  ...
  async linkMemoryEntity(
    ...
    try {
      ...
      await this.client.query(
        `UPDATE ${ENTITY_TABLE}:${entityId} SET relation_count += 1`,
        {}
      );
   ...
   */
  async getMemoriesByEntity(entityId: number, limit: number = 100): Promise<LinkedMemory[]> {
    if (!this.client) {
      throw new Error('[SurrealDB] Client not connected');
    }
    ...
    try {
      const result = await this.client.query(
        `SELECT * FROM memory_entity WHERE out = entity:${entityId} ORDER BY relevance DESC LIMIT ${limit}`,
        {}
      );
  ...
  async getTopicsByEntity(entityId: string | number): Promise<any[]> {
    ...
    try {
      ...
      const result = await this.client.query(
        `SELECT * FROM ${TOPIC_TABLE} WHERE parent_entity_id = $entityId ORDER BY created_at DESC`,
        { entity_id: numericId }
      );
  ...
  async resolveAlias(alias: string, visited: Set<string> = new Set()): Promise<string | null> {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const canonicalResult = await this.client.query(
  ...
  async mergeEntities(
    ...
    try {
      ...
      const aliasMemoriesResult = await this.client.query(
      ...
      await this.client.query(
        `UPDATE entity:${aliasId} SET canonical_id = ${canonicalId}, is_merged = true, merged_at = time::now()`,
        {}
      );
  ...
  async getEntityStats(entityId: string | number): Promise<{ memory_count: number; topic_count: number }> {
    ...
    try {
      ...
      const memoryResult = await this.client.query(
        `SELECT count() as count FROM memory_entity WHERE out = entity:${numericId} GROUP ALL`,
        {}
      );
      ...
      // Count topics
      const topicResult = await this.client.query(
        `SELECT count() as count FROM ${TOPIC_TABLE} WHERE parent_entity_id = ${numericId} GROUP ALL`,
        {}
      );
  ...
  async buildEntityCooccurrence(batchSize: number = 1000): Promise<number> {
    ...
    for (const [pairKey, pairData] of cooccurrenceMap.entries()) {
      ...
      const entityARecord = await this.client.query(
        `SELECT mention_count FROM ${ENTITY_TABLE}:${entityA}`,
        {}
      );
      const entityBRecord = await this.client.query(
  ...
  async pruneLowWeightEdges(minWeight: number = 0.1): Promise<number> {
    ...
    const beforeResult = await this.client.query(
  ...
  async getRelationStats(): Promise<{
    ...
    try {
      ...
      const weightResult = await this.client.query(
