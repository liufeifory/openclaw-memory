...
class MockMemoryManager {
  ...
  async storeSemantic(content: string, importance: number, sessionId: string) {
...
async function testWatcherCreation() {
  ...
  const mockMM = new MockMemoryManager() as any;
...
async function testSupportedExtensions() {
  ...
  const supportedExt = (watcher as any).supportedExtensions;
