...
import { HybridRetriever, MemoryResult } from './hybrid-retrieval.js';
...
class MockReranker {
  ...
  async rerank(query: string, results: any[], options?: any): Promise<any[]> {
  ...
  applyDiversityPenalty(results: any[]): any[] {
    return results;
  }
}
...
let db: SurrealDatabase;
let embedding: any;
let reranker: any;
let entityIndexer: EntityIndexer;
let topicIndexer: TopicIndexer;
...
async function testTopicSchema(): Promise<boolean> {
...
async function testTopicCRUD(): Promise<boolean> {
  console.log('\n=== Test 2: Topic CRUD Operations ===');
  ...
  try {
    ...
    const memoryId = await db.upsert(
      999,
      new Array(1024).fill(0),
      { content: 'Test memory content', type: 'episodic' }
    );
...
async function testTopicRecall(): Promise<boolean> {
  console.log('\n=== Test 3: Topic Recall Retrieval ===');
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      const memoryId = await db.upsert(
        1000 + i,
        new Array(1024).fill(i * 0.1),
        { content: `Test memory ${i} for retrieval`, type: 'episodic' }
      );
...
async function test4PathMerge(): Promise<boolean> {
  console.log('\n=== Test 4: 4-Path Merge with Priority Scoring ===');

  try {
    // Create test memories with different sources
    const vectorResults: MemoryResult[] = [
    ...
    const graphResults: MemoryResult[] = [
    ...
    const topicResults: MemoryResult[] = [
...
async function testAliasResolution(): Promise<boolean> {
  console.log('\n=== Test 5: Alias Resolution ===');
...
async function testEntityMerge(): Promise<boolean> {
  console.log('\n=== Test 6: Entity Merge ===');
  ...
  try {
    ...
    const memoryId = await db.upsert(
      ...
      { content: 'Memory linked to Postgres', type: 'episodic' }
    );
    ...
    assert(aliasData?.canonical_id === expectedCanonicalId || aliasData?.is_merged === true,
      'Alias entity marked as merged');
...
async function testTopicAutoCreation(): Promise<boolean> {
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      const mid = await db.upsert(
        3000 + i,
        new Array(1024).fill(i * 0.05),
        { content: `Memory ${i} about SuperNodeEntity topic ${i % 3}`, type: 'episodic' }
      );
...
async function testFullRetrievalPipeline(): Promise<boolean> {
  ...
  try {
    ...
    // Create memories
    for (let i = 0; i < 3; i++) {
      const memId = await db.upsert(
        4000 + i,
        new Array(1024).fill(0.3 + i * 0.1),
        { content: `Pipeline test memory ${i}`, type: 'episodic' }
      );
